  ...
  return (
    <div className=' p-4 border-[1px] mt-5 md:ml-5 rounded-lg '>
        ...
        {doctorList.map((doctor:any,index)=>(
            <Link key={index} href={'/details/'+doctor?.id} className=' mb-4 p-3 shadow-sm w-full 
            ...
            rounded-lg flex items-center gap-3'>
                ...
                <div className='mt-3 flex-col flex gap-1 items-baseline'>
                    <h2 className='text-[10px] bg-blue-100 p-1 rounded-lg px-2
                ...
                <h2 className='text-primary text-xs flex gap-2'>
                    {/* <GraduationCap/> */}
                    {doctor.attributes.Years_of_Experience}</h2>
                    </div>
            </Link>
        ))}
    </div>
